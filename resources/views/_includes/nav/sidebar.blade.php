<aside class="menu notification is-info">
  ...
  @if (Auth::user()->hasRole(['admin', 'editor']))
    ...
    <p class="menu-label">Üzenetek</p>
    <ul class="menu-list">
      <!-- <li><a {{ ( strpos(Route::currentRouteName(), 'notifications') > -1) ? 'class=is-active' : '' }} href="{{ route('notification.list') }}">Üzenetek</a></li> -->
      <li><a {{ ( strpos(Route::currentRouteName(), 'jobs') > -1) ? 'class=is-active' : '' }} href="{{ route('jobs.list') }}">Feladatok</a></li>
    </ul>
